...
## figure defaults to current roman option
## use link = '...' to link to anchor ... 
## link defaults to t1 or f1 for table or figure + current number
...
As we saw in Figure `r I(figurenum_ - 1)`, something something, and we can also see in `r get_num('figure', link = TRUE, roman = TRUE)`, something.
...
pathr <- paste0(projdir, '00-master.Rmd')
pathh <- paste0(projdir, '00-master.html')
path_newr <- paste0(projdir, 'reports/',
                    paste(as.character(format(Sys.Date(), format = '%d%b%Y'))),
                    '.Rmd')
path_newh <- paste0(projdir, 'reports/',
                    paste(as.character(format(Sys.Date(), format = '%d%b%Y'))),
                    '.html')
